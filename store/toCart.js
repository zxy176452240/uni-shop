export default {
  ...
  mutations: {
    ...
    addToCart(state, item) {
      ...
      // find() 查找满足条件的属性
      const findResult = state.cart.find(x => x.goods_id === item.goods_id)
  ...
  getters: {
    ...
    checkedGoodsAmount(state) {
      ...
      // 调用 toFixed(2) 方法，保留两位小数
      // 调用 replace() 方法，进行千分位格式化
      return state.cart.filter(x => x.goods_state)
